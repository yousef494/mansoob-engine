...
local_file = "data.wtli"
state_file = "state.wtli"
prcessing_interval = 300  #in seconds,  300s = 5m
set_prcessing_interval = 300  #in seconds,  300s = 5m
r_prcessing_interval = 10  #in seconds,  300s = 5m
info_log = "info_log.wtli"
error_log = "error_log.wtli"
warning_log = "warning_log.wtli"
stat_log = "stat_log.wtli"
test_log = "test_log.wtli"
...
def init():
	prop.load(open('wtli.prop'))
	
	global tank_hight
	tank_hight = int(prop['tank_hight'])
	global sender
	sender = prop['sender']
	global reciever
	reciever = prop['reciever']
	
	global normal_min
	normal_min = int(prop['normal_min'])
	global low_min
	low_min = int(prop['low_min'])
	global medium_min
	medium_min = int(prop['medium_min'])
	global high_min
	high_min = int(prop['high_min'])
	
	global normal_alert
	normal_alert = prop['normal_alert']
	global low_alert
	low_alert = prop['low_alert']
	global medium_alert
	medium_alert = prop['medium_alert']
	global high_alert
	high_alert = prop['high_alert']
	global critical_alert
	critical_alert = prop['critical_alert']
	
	global reporting_time_1
	reporting_time_1 = prop['reporting_time_1']
	global reporting_time_2
	reporting_time_2 = prop['reporting_time_2']
	
	state = readLastState()
	global global_pdata
	global_pdata = state[1]
	global global_severity
	global_severity = state[2]
	
	
def run_command():
	log("info","run_command:starting the script...")
	log("","run_command:starting the script...")
	set_init_level()
	rest_prcessing_interval(set_prcessing_interval)
	output = read_currentLevel()
	if output:
		if(store(output.strip()) == False):
			rest_prcessing_interval(r_prcessing_interval)
	return output
...
def read_currentLevel():
	#arduino = arduinoserial.SerialPort(file, 9600)
	#output = arduino.read_until('\n')
	output = '76'
	return output
    	


def invalide(data,adata):
	try:
		if(data < 0):
			return True
		if(data - adata < -10):
		...
		if(data - adata > 10):
			return True
		if(data > tank_hight-12):
			return True
		global global_pdata
		global_pdata = data
		log("info","invalide:valid data... "+str(data))
		return False
	except Exception, e:
		log("error","invalide:Error while validating the data..."+str(e))
		return True
...
def set_init_level():
	global global_pdata
	log("info","set_init_level:getting the initial level...")
	log("","set_init_level:getting the initial level...")
	i = 0
	a = array("i")
	while i < 4:
		output = read_currentLevel()
		log("test",output)
		if output:
			data = normalize(output.strip()) 
			if(data != "NAN"):
				if(invalide(int(data),int(data)) == False):
					a.append(int(data))
					i = i+1

	global_pdata = a[0]#most_common.most_common(a)
	log("","set_init_level:setting initial level..."+str(global_pdata))
	return 0
...
def store(sdata):
	global global_pdata
	global global_severity
	data = normalize(sdata)
	if(data == "NAN"):
		return False
	if(data != "NAN"):
		invalid = invalide(int(data),global_pdata)
		if(invalid == True):
			global counter
			counter = counter + 1
			if(counter > 3):
			...
			return False
		if(invalid == False):
			global_pdata = int(data)
			timestamp = str(datetime.now())
			storeToDataset(timestamp,data)
			current_severity = calculate_severity(data)
			send_Alert(data,current_severity,global_severity)
			send_Report(data,current_severity,global_severity)
			storeState(data,current_severity)
			counter = 0
			return True
			

def rest_prcessing_interval(m):
	global prcessing_interval
	#log("info","rest_prcessing_interval:reset prcessing_interval "+ str(m))
	prcessing_interval = m;
...
def normalize(data):
	try:
		#r = data.split(",")
		norm = tank_hight - int(data)
		return str(norm)
	except:
		log("warning","normalize:NAN, Problem in resolving the data... "+data)
		return str("NAN")

	

def storeToDataset(timestamp,data):
	with open(local_file, "a") as ofile:
...
def readLastState():
	try:
		logTime = str(datetime.now())
		with open(state_file, "rb") as ofile:
			lines = ofile.readlines()
			last_line = lines[-1]
			last_line = last_line.split(",")
			return last_line
	except Exception, e:
		return ['2016-10-06 18:16:49.446538',0,'Normal']
...
def calculate_severity(data):
	severity = "Normal"
	try:
		if(int(data) > normal_min):
			severity = "Normal"
		if(int(data) < normal_min):
			severity = "Low"
		if(int(data) < low_min):
			severity = "Medium"
		if(int(data) < medium_min):
			severity = "High"
		if(int(data) < high_min):
			severity = "Critical"
		return severity
	except Exception, e:
		return severity
...
def send_Alert(data, current_severity, g_severity):
	try:
		message = "";
		if(current_severity == "Low" and g_severity == "Normal"):
			prepare_AlertMessage(data,"Low","#f0ad4e")
		if(current_severity == "Medium" and g_severity == "Low"):
			prepare_AlertMessage(data,"Medium","#ff8d00")
		if(icurrent_severity == "High" and g_severity == "Medium"):
			prepare_AlertMessage(data,"High","#d9534f")
		if(current_severity == "Critical" and g_severity == "High"):
			prepare_AlertMessage(data,"Critical","#8b0202")
			
		if(message != "" or message != "ERROR"):
			do_smtp("WTLI Status Alarm Test", sender, [receiver], message)
			log("info","send_Alert:Eamil has been sent for severity alarm "+current_severity)
	except Exception, e:
		log("error","send_Alert:"+str(e))
		
		
def prepare_AlertMessage(data,severity,color):
	try:
		html = """\
...
def do_smtp1(subject, email_sender, email_receivers, message):
	try:
		...
		log("info","do_smtp:Successfully sent email...")         
	except Exception, e:
		log("error","do_smtp:error while communicating to smtp server..."+str(e)) 

	
def do_smtp(subject, email_sender, email_receivers, message):
	try:
		host = 'smtp.live.com'
		port = "25"
		sender = email_sender
		...
		msg = MIMEMultipart('alternative')
		msg['Subject'] = 'WTLI Summary Report'
		msg['From'] = sender
		msg['To'] = ', '.join(receivers)
		text = "Hi!\nPlease notice that the latest level was rcorded is ("+message+").\n Check the link for more details http://wtli.freeiz.com/wtli  ."
		html = message
		part1 = MIMEText(text, 'plain')
		part2 = MIMEText(html, 'html')
		msg.attach(part1)
		...
		smtpObj.ehlo()
		smtpObj.starttls()
		smtpObj.login(sender, "a6482@Hotmail1")
		smtpObj.sendmail(sender, receivers, msg.as_string())     
	except Exception, e:
		print "",e	
...
def log(type, message):
	logTime = str(datetime.now())
	if(type == "info"):
		with open(info_log, "a") as ofile:
			ofile.write(logTime+":"+message)
			ofile.write("\n")
			return
	if(type == "warning"):
		with open(warning_log, "a") as ofile:
			ofile.write(logTime+":"+message)
			ofile.write("\n")
			return
	if(type == "error"):
		with open(error_log, "a") as ofile:
			ofile.write(logTime+":"+message)
			ofile.write("\n")
			return
	if(type == "stat"):
		with open(stat_log, "a") as ofile:
			ofile.write(logTime+":"+message)
			ofile.write("\n")
			return
	if(type == "test"):
		with open(test_log, "a") as ofile:
			ofile.write(logTime+":"+message)
			ofile.write("\n")
			return
	else:
